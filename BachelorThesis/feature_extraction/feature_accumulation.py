...
def accumulate_total_emotion_vectors(collected_features, collected_extra_features):
...
def accumulate_emotion_entropies(collected_features, collected_extra_features):
    focus_feature = collected_features[0]
    emotion_vectors = np.sum(focus_feature, axis=1, dtype=np.uint8)
...
def accumulate_emotion_synchronicities(collected_features, collected_extra_features):
...
def accumulate_lip_movement(collected_features, collected_extra_features):
...
def accumulate_interactions(collected_features, collected_extra_features):
    ...
    for i in range(focus_feature.shape[0]-1):
        ...
        for j in range(i+1,focus_feature.shape[0]):
            ...
            for k in range(len(directions_i)):
                ...
                interaction_ij = detect_interaction(origin_x_i, origin_y_i, m_i, d_i, lower_x_j, upper_x_j, lower_y_j, upper_y_j)
                interaction_ji = detect_interaction(origin_x_j, origin_y_j, m_j, d_j, lower_x_i, upper_x_i, lower_y_i, upper_y_i)
...
def accumulate_activeness(collected_features, collected_extra_features):
...
def accumulate_people_proximities(collected_features, collected_extra_features):
    focus_feature = collected_features[1]
...
def accumulate_pose_synchronicities(collected_features, collected_extra_features):
    focus_feature = collected_features[1]
...
def calculate_pose_angles(keypoints):
    ...
    connections = np.array([[left_forearm, left_upper_arm], [-left_upper_arm, left_side], [-left_side, -left_thigh],
                   ...
                   [right_forearm, right_upper_arm], [-right_upper_arm, right_side], [-right_side, -right_thigh],
